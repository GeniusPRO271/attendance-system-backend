...
import type { subjectFilterSchemaType } from "../zod/select_schema";
...
export interface SubjectService {
  ...
  getAllFromQuery(filters: subjectFilterSchemaType): Promise<SubjectDTO[]>; // Returns an array of basic subject data
}
...
export class SubjectServiceClass implements SubjectService {
  ...
  async getAllFromQuery(filters: subjectFilterSchemaType): Promise<SubjectDTO[]> {
    const conditions = []; // Array to hold filter conditions

    if (filters.id) {
      conditions.push(eq(SubjectTable.id, filters.id));
    }
    if (filters.name) {
      conditions.push(like(SubjectTable.name, `%${filters.name}%`));
    }
    if (filters.group_id) {
      conditions.push(exists(
        this.db.select({ _: sql`1` })
          .from(subjectsToGroupsTable)
          .where(and(
            eq(subjectsToGroupsTable.subject_id, SubjectTable.id),
            eq(subjectsToGroupsTable.group_id, filters.group_id)
          ))
      ));
    }

    const baseQuery = this.db.select().from(SubjectTable);
    ...
    const subjectsData = await finalQuery;

    console.log(subjectsData)
    return subjectsData as SubjectDTO[];
  }
